...
- Memory (BroccoliDB) — bundled versioned long-term memory plugin (auto-recall/branching; set `plugins.slots.memory = "memory-broccolidb"`)
- Memory (LanceDB) — legacy bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
...
- Bundled extension-specific subpaths are also available:
  ...
  `openclaw/plugin-sdk/memory-broccolidb`,
