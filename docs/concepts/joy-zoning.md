...
These strikes are more than just logs; they exert *backpressure* directly into the agent's context window. If an agent tries to hack around a boundary, the strike warns it, and it will autonomously course-correct its implementation plan to respect the architecture.
