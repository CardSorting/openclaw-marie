...
import { getJoyZoningStore, type JoyZoningHealthSummary } from "../infra/joy-zoning-store.js";
...
export function redactSensitiveStatusSummary(summary: StatusSummary): StatusSummary {
  return {
    ...
    joyZoning: summary.joyZoning ? { ...summary.joyZoning, topOffenders: [] } : null,
  };
}
...
export async function getStatusSummary(
  ...
  let joyZoning: JoyZoningHealthSummary | null = null;
  try {
    const defaultStore = getJoyZoningStore();
    joyZoning = defaultStore.getHealthSummary();
  } catch {
    // Advisory feature, proceed if unavailable
  }
