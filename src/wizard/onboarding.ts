...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Marie is a hobby project and still in beta. Expect sharp edges.",
      "By default, Marie is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Marie is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Marie.",
      ...
      "- Shared inboxes: isolate DM sessions (`session.dmScope: per-channel-peer`) and keep tool access minimal (now enabled by default for new setups).",
      "- Keep secrets out of the agent’s reachable filesystem.",
      "- Use the strongest available model for any bot with tools or untrusted inboxes.",
      "",
      "Run regularly:",
      "marie security audit --deep",
      "marie security audit --fix",
      "",
      "Must read: https://docs.marieai.com/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Marie onboarding");
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.marieai.com/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("marie doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("marie configure")}.`;
