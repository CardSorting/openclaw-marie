...
export async function sweepCronRunSessions(params: {
  ...
  // Prune Joy-Zoning store using the same retention window
  try {
    ...
    if (prunedViolations > 0 || prunedJzSessions > 0 || decayedStrikes > 0) {
      params.log.info(
        { prunedViolations, prunedJzSessions, decayedStrikes, retentionMs },
        `cron-reaper: jz-maintenance: pruned ${prunedViolations} violations, ${prunedJzSessions} sessions, decayed ${decayedStrikes} strikes`,
      );
    }
  } catch (err) {
    params.log.warn({ err: String(err) }, "cron-reaper: Joy-Zoning cleanup failed");
  }
