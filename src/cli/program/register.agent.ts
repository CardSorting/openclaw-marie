...
${formatHelpExamples([
  ['marie agent --to +15555550123 --message "status update"', "Start a new session."],
  ['marie agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'marie agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'marie agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['marie agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'marie agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.marieai.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.marieai.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['marie agents set-identity --agent main --name "MARIE" --emoji "🤖"', "Set name + emoji."],
  ["marie agents set-identity --agent main --avatar avatars/marie.png", "Set avatar path."],
  [
    "marie agents set-identity --workspace ~/.marie/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "marie agents set-identity --identity-file ~/.marie/workspace/IDENTITY.md --agent main",
