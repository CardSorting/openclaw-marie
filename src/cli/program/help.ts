...
const EXAMPLES = [
  ...
  ["marie gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["marie --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["marie gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["marie gateway ...", "Gateway control via WebSocket."],
  [
    'marie agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'marie message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.marieai.com/cli");
