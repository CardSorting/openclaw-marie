  ...
  jz.command("health")
    ...
    .action(async () => {
      try {
        ...
        defaultRuntime.log(`Hard Blocks:      ${health.totalBlocks > 0 ? danger(health.totalBlocks) : success("0")}`);
        defaultRuntime.log(`Files w/ Strikes: ${theme.accent(health.filesWithStrikes.toString())}\n`);

        if (health.topOffenders.length > 0) {
          defaultRuntime.log(theme.heading("Top Offenders:"));
          for (const off of health.topOffenders) {
            defaultRuntime.log(`  - ${off.filePath}: ${theme.warn(off.strikeCount.toString())} strikes`);
          }
          defaultRuntime.log("");
        } else {
  ...
  jz.command("prune")
    ...
    .action(async (opts: { violations: string }) => {
      try {
        ...
        const prunedViolations = store.pruneViolations(keepCount);
