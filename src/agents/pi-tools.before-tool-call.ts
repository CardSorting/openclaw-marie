    ...
    if (filePath || newPath) {
      ...
      if (jzResult) {
        if (jzResult.level === "block") {
          const hint = jzResult.correctionHint ? `\n${jzResult.correctionHint}` : "";
          log.error(`Joy-Zoning blocked ${toolName}: ${jzResult.message}`);
          return { blocked: true, reason: `[Joy-Zoning] ${jzResult.message}${hint}` };
        }
        log.warn(`Joy-Zoning warning for ${toolName}: ${jzResult.message}`);
      }
    }
  } catch {
    // Joy-Zoning is advisory; never break tool execution if it fails to load.
  }
