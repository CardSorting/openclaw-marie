...
export async function handleToolExecutionEnd(
  ...
  // Clear Joy-Zoning strikes when a file is successfully mutated (meaning the agent fixed the violations)
  if (!isToolError && ["write", "edit", "patch"].includes(toolName) && ctx.params.sessionKey) {
    const filePath =
      typeof afterToolCallArgs.path === "string"
        ...
        : typeof afterToolCallArgs.file_path === "string"
          ...
          : undefined;
    if (filePath) {
      clearStrikesForFile(ctx.params.sessionKey, filePath);
    }
  }
