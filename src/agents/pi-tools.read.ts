...
export function createOpenClawReadTool(
  ...
  return {
    ...
    execute: async (toolCallId, params, signal) => {
      ...
      // Prepend Joy-Zoning layer context
      const withContext = await (async () => {
        try {
          const { getConfig, detectLayer } = await import("./joy-zoning.policy.js");
          const config = getConfig();
          if (config?.enabled !== false && filePath !== "<unknown>") {
            const layer = detectLayer(filePath);
            if (layer) {
              const text = getToolResultText(strippedDetailsResult);
              if (typeof text === "string" && !text.startsWith("[Joy-Zoning Context:")) {
                const header = `[Joy-Zoning Context: ${layer.toUpperCase()} LAYER]\n`;
                return withToolResultText(strippedDetailsResult, header + text);
              }
            }
          }
        } catch {
          // Ignore import/resolution errors silently during read
        }
        return strippedDetailsResult;
      })();

      const normalizedResult = await normalizeReadImageResult(withContext, filePath);
