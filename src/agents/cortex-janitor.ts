...
export class CortexJanitor {
   ...
   */
  public static runHygiene(content: string, sessionKey?: string): { cleaned: string; strippedCount: number } {
    let cleaned = content;
    let strippedCount = 0;
    ...
    if (sessionKey) {
        try {
            ...
            if (keptLines.length < lines.length) {
                log.info(`Recall-Based Ablation: Pruned ${lines.length - keptLines.length} forgotten lines.`);
