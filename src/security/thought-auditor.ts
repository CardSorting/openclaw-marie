...
const MALICIOUS_INTENTS = [
  {
    pattern: /jailbreak|ignore.*?instruction|system.*?prompt.*?override/i,
  ...
  {
    pattern: /ssh-keygen|cat.*?\.ssh|authorized_keys|\/etc\/passwd|\/etc\/shadow/i,
    intent: "unauthorized-access",
  },
  { pattern: /curl.*?\|.*?bash|wget.*?\|.*?sh|nc\s+-l/i, intent: "remote-code-execution" },
  {
    pattern: /social.*?engineering|manipulate.*?user|pretend.*?to.*?be/i,
...
export function auditThoughts(thought: string): ThoughtAuditResult {
  ...
  for (const entry of MALICIOUS_INTENTS) {
    if (entry.pattern.test(thought)) {
      log.warn(
        `Suspicious thought patterns detected [${entry.intent}]: ${thought.substring(0, 100)}...`,
      );
      return {
        suspicious: true,
