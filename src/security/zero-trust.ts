...
export function authorizeToolExecution(command: string): { authorized: boolean; reason?: string } {
  ...
  const isSuspicious =
    health.totalWarnings > 0 || health.totalBlocks > 0 || health.filesWithStrikes > 0;

  if (isSuspicious) {
    const highRiskTools = /\b(nc|netcat|nmap|ssh|scp|ftp|rsync|base64|xxd|sudo)\b/i;
    if (highRiskTools.test(command)) {
      log.warn(
        `Zero-Trust Block: Prevented execution of high-risk tool '${command}' in suspicious session.`,
      );
      return {
        authorized: false,
